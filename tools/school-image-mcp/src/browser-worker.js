...
export async function browserWorker(port = 18765) {
  ...
  if (window.__schoolImageWorker?.active) return;
  const state = window.__schoolImageWorker = { active: true, completed: 0, lastError: null };
  const base = `http://127.0.0.1:${port}`;
  try {
    const { workerToken } = await (await fetch(base + '/connect')).json();
  ...
  } catch { state.lastError = 'BRIDGE_DISCONNECTED'; }
