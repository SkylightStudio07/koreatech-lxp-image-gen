...
const listed=await client.listTools();assert.equal(listed.tools.length,21);assert(listed.tools.some(x=>x.name==='propose_edit'));assert(listed.tools.some(x=>x.name==='read_asset_metadata'));assert(listed.tools.some(x=>x.name==='read_instructions'));assert(listed.tools.some(x=>x.name==='read_skill'));assert(listed.tools.some(x=>x.name==='notion_search'));assert(listed.tools.some(x=>x.name==='unity_build'));assert(listed.tools.some(x=>x.name==='unity_set_component'));
