...
const listed=await client.listTools();assert.equal(listed.tools.length,49);assert(listed.tools.some(x=>x.name==='create_directory'));assert(listed.tools.some(x=>x.name==='save_image_asset'));assert(listed.tools.some(x=>x.name==='propose_edit'));assert(listed.tools.some(x=>x.name==='read_asset_metadata'));assert(listed.tools.some(x=>x.name==='list_visual_assets'));assert(listed.tools.some(x=>x.name==='read_image'));assert(listed.tools.some(x=>x.name==='list_model_assets'));assert(listed.tools.some(x=>x.name==='read_model_metadata'));assert(listed.tools.some(x=>x.name==='read_instructions'));assert(listed.tools.some(x=>x.name==='read_skill'));assert(listed.tools.some(x=>x.name==='run_shell'));assert(listed.tools.some(x=>x.name==='start_background_task'));assert(listed.tools.some(x=>x.name==='task_status'));assert(listed.tools.some(x=>x.name==='notion_search'));assert(listed.tools.some(x=>x.name==='unity_build'));assert(listed.tools.some(x=>x.name==='unity_set_component'));assert(listed.tools.some(x=>x.name==='unity_capture_scene'));assert(listed.tools.some(x=>x.name==='unity_project_status'));assert(listed.tools.some(x=>x.name==='unity_instantiate_prefab'));
