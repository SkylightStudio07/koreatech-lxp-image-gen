...
const listed=await client.listTools();assert.equal(listed.tools.length,7);assert(listed.tools.some(x=>x.name==='propose_edit'));assert(listed.tools.some(x=>x.name==='read_asset_metadata'));assert(listed.tools.some(x=>x.name==='read_instructions'));
