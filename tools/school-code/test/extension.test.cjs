...
test('extension: chat stream, conversation continuation, approved edits, refusal and stale hash',async t=>{
  ...
  const info=await client.callTool({name:'workspace_info',arguments:{}});const infoData=JSON.parse(info.content[0].text);assert.deepEqual(infoData.tools,['list_files','read_file','search_text','read_asset_metadata','read_instructions','read_skill','notion_search','notion_fetch_page','notion_list_children','unity_project_info','unity_run_tests','unity_build','unity_refresh_assets','unity_open_scene','unity_find_gameobjects','unity_get_component','unity_set_component','unity_create_gameobject','unity_save_scene','propose_edit']);assert.equal(infoData.root_isolation,true);assert.deepEqual(infoData.skills.skills,[]);
