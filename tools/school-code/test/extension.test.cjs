...
test('extension: chat stream, conversation continuation, approved edits, refusal and stale hash',async t=>{
  ...
  const commands=new Map(),posts=[],errors=[],inputs=[url,workerToken],ctx={subscriptions:[],extensionUri:'extension',workspaceState:{get:(k,f)=>f,update:async()=>{}},secrets:{get:async()=>null,store:async()=>{}}};let provider,handler,bridge,approved=true,approvalHook=null,approvalCount=0;
  ...
  let localState=posts.filter(p=>p.type==='state').at(-1);assert.equal(localState.state.goal.text,'Fix the player movement bug');assert.equal(localState.state.goal.status,'active');
  await handler({type:'send',message:'/goal done'});localState=posts.filter(p=>p.type==='state').at(-1);assert.equal(localState.state.goal.status,'completed');
  ...
  const sending=handler({type:'send',message:'hello',model:'gpt-6-astra',mode:'direct'});const job=await nextJob();assert.equal(job.body.direct,true);assert.equal(job.body.conversation_id,undefined);assert(job.body.message.includes('hello.txt'));assert(job.body.message.includes('[현재 작업 목표]'));assert(job.body.message.includes('Verify the fix with tests'));assert(!job.body.message.includes(root));assert.equal(posts.filter(p=>p.type==='state').at(-1).projectContext.files.length,0);
  ...
  const approvalsBeforeInfo=approvalCount;const info=await client.callTool({name:'workspace_info',arguments:{}});const infoData=JSON.parse(info.content[0].text);assert.equal(approvalCount,approvalsBeforeInfo);assert.deepEqual(infoData.tools,['create_directory','list_files','read_file','search_text','read_asset_metadata','read_instructions','read_skill','run_shell','start_background_task','task_status','task_output','task_cancel','notion_search','notion_fetch_page','notion_list_children','unity_project_info','unity_run_tests','unity_build','unity_refresh_assets','unity_open_scene','unity_find_gameobjects','unity_get_component','unity_set_component','unity_create_gameobject','unity_save_scene','propose_edit']);assert.equal(infoData.root_isolation,true);assert.equal(infoData.instructions_missing,false);assert.equal(infoData.instructions_setup.created,true);assert.equal(infoData.instructions_setup.automatic,true);assert.deepEqual(infoData.instruction_files,['AGENTS.md']);assert.match(await fs.readFile(path.join(root,'AGENTS.md'),'utf8'),/created automatically/);assert.deepEqual(infoData.skills.skills,[]);
  const directory=await client.callTool({name:'create_directory',arguments:{path:'Generated/Textures'}});assert(!directory.isError,directory.content?.[0]?.text);assert.equal((await fs.stat(path.join(root,'Generated/Textures'))).isDirectory(),true);
  ...
  const unityInfo=await client.callTool({name:'unity_project_info',arguments:{}});assert.equal(JSON.parse(unityInfo.content[0].text).name,path.basename(root));
...
test('extension routes workflow agents through workflow SSE and records steps',async t=>{
  const root=await fs.mkdtemp(path.join(os.tmpdir(),'school-workflow-'));t.after(()=>fs.rm(root,{recursive:true,force:true}));
  const commands=new Map(),posts=[],errors=[],ctx={subscriptions:[],extensionUri:'extension',workspaceState:{get:(k,f)=>f,update:async()=>{}},secrets:{get:async()=>null,store:async()=>{}}};let provider,handler,bridge;
  const uri=p=>({fsPath:p,scheme:'file',toString:()=>p});
  const vscode={Uri:{parse:uri,file:uri,joinPath:(base,...p)=>uri([base,...p].join('/'))},Range:class{},WorkspaceEdit:class{},env:{clipboard:{writeText:async()=>{}},openExternal:async()=>true},window:{createOutputChannel:()=>({appendLine(){},dispose(){}}),showInformationMessage:async()=>{},showErrorMessage:e=>errors.push(e),showWarningMessage:async()=> '승인',showInputBox:async()=>'',showTextDocument:async()=>{},registerWebviewViewProvider:(id,p)=>{provider=p;return {dispose(){}};}},workspace:{isTrusted:true,workspaceFolders:[{name:'fixture',uri:uri(root)}],textDocuments:[],getConfiguration:()=>({get:(key,f)=>key==='bridgePort'?0:f}),registerTextDocumentContentProvider:()=>({dispose(){}}),openTextDocument:async u=>({uri:u,getText:()=>'',positionAt:i=>i,save:async()=>true}),applyEdit:async()=>true},commands:{registerCommand:(id,fn)=>{commands.set(id,fn);return {dispose(){}};},executeCommand:async()=>{}}};
  const requireLocal=createRequire(path.resolve(__dirname,'../src/extension.cjs'));const module={exports:{}};
  vm.runInNewContext(await fs.readFile(path.resolve(__dirname,'../src/extension.cjs'),'utf8'),{module,exports:module.exports,require:id=>id==='vscode'?vscode:id==='./panel.cjs'?{panelHtml:()=>'<html></html>'}:id==='./bridge.cjs'?{BrowserBridge:class extends BrowserBridge{constructor(port){super(port);bridge=this;}}}:requireLocal(id),Buffer,URL,fetch,AbortController,AbortSignal,setInterval,clearInterval,setTimeout,clearTimeout,console});
  module.exports.activate(ctx);t.after(()=>ctx.subscriptions.forEach(s=>s.dispose()));await until(()=>bridge.server?.listening);provider.resolveWebviewView({webview:{options:{},postMessage:m=>posts.push(structuredClone(m)),onDidReceiveMessage:fn=>{handler=fn;return {dispose(){}};}},onDidDispose:()=>{}});
  const base=`http://127.0.0.1:${bridge.port}`,headers={Origin:'https://ai.koreatech.ac.kr',Authorization:'Bearer '+bridge.key,'Content-Type':'application/json'};await fetch(base+'/worker/heartbeat',{headers});async function nextJob(){return (await fetch(base+'/worker/poll',{headers})).json();}async function event(data){await fetch(base+'/worker/event',{method:'POST',headers,body:JSON.stringify(data)});}
  const refreshing=handler({type:'refresh'});for(const result of [{items:[{id:'gpt-6-sol',available:true}]},{items:[{id:'school-agent',name:'School Code 프로젝트 개발자',has_workflow:true}]},{items:[]}]){const job=await nextJob();await event({id:job.id,result,done:true});}await refreshing;
  const sending=handler({type:'send',message:'프로젝트를 분석해줘',agent:'school-agent',mode:'default'});const job=await nextJob();assert.equal(job.route,'/agents/school-agent/workflow/run');assert.equal(job.body.stream,true);assert.match(job.body.input_text,/프로젝트를 분석해줘/);
  await event({id:job.id,chunk:'data: {"type":"run_start","run_id":"run-1"}\n\ndata: {"type":"node_output","node_name":"MCP 도구 실행","output":"파일을 확인했습니다."}\n\ndata: {"type":"token","content":"완료"}\n\ndata: {"type":"run_end","output":"최종 답변"}\n\n'});await event({id:job.id,done:true});await sending;
  const state=posts.filter(p=>p.type==='state').at(-1);const answer=state.state.messages.at(-1);assert.equal(answer.status,'완료');assert.match(answer.text,/완료/);assert.equal(answer.steps.length,3);assert.deepEqual(errors,[]);
});
