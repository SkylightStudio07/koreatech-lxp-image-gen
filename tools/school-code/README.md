...
일반 사용자는 저장소를 내려받아 빌드하지 말고 GitHub Release에서 `school-code-0.20.0.vsix`와 `school-code-connector-0.3.0.zip`을 설치하세요. 이 문서는 기능·운영·개발 세부사항을 위한 문서이며, 구성원용 순서도는 저장소 루트의 [forBCSD.md](https://github.com/SkylightStudio07/koreatech-lxp-image-gen/blob/master/forBCSD.md)를 따릅니다.
...
School Code는 Codex·Claude Code처럼 프로젝트 전체를 처음부터 대화에 올리지 않고, 모델이 필요한 구조를 도구로 조사한 뒤 수정하도록 동작합니다. 작업 요청을 받으면 `workspace_info`와 프로젝트 지침을 확인하고, 관련 디렉터리·대상 파일·호출부·심볼을 읽은 다음 작은 diff를 제안해야 합니다. 프로젝트에 없는 API·필드·enum·싱글턴을 추측하지 않으며, 기존 파일 전체를 새로 쓰지 않습니다. 수정 후에는 Unity 컴파일·테스트·검증 도구를 실행하고 오류가 나면 실제 선언부와 호출부를 다시 확인합니다.

이 순서는 서버가 모든 읽기 호출을 강제로 검사하는 방식이 아닙니다. 읽기·검색은 작업에 필요한 만큼 수행하고, 서버는 프로젝트 경로·SHA-256·승인·위험한 명령 같은 안전 경계만 지킵니다. `propose_edit`는 VS Code diff를 먼저 열며 변경 요약·검증 계획과 넓은 범위 변경 경고를 함께 보여 줍니다. 승인하지 않은 수정은 저장되지 않습니다.

## 설치와 채팅

1. VS Code에 `school-code-0.20.0.vsix`를 설치합니다.
...
- 워크플로우가 `run_end`와 단계 타임라인만 남기고 본문을 표시하지 않으면 **보기 → 출력 → School Code**를 열어 `[workflow] run_end text_length=… accumulated_length=… node_output_length=… output_shape=…`를 확인하세요. 세 값이 모두 0이면 학교 Workflow API가 최종 텍스트를 보내지 않은 것이고, 하나라도 양수인데 화면만 비면 확장·패널 문제입니다. 로그에는 토큰·답변 본문·Notion 내용이 기록되지 않습니다.
- 답변이 생성 중일 때는 마지막 메시지에 회전 표시와 `답변 생성 중…`이 보입니다. 답변 본문은 안전한 Markdown으로 표시되어 굵게·기울임·제목·목록·인라인 코드·링크·인용문을 읽기 쉽게 렌더링합니다.
- 학교 요청이 응답을 받기 전에 일시적인 네트워크 오류로 끊기면 최대 3회 자동 재시도합니다. 1초·2초·4초 간격으로 시도하며, 응답이 이미 시작됐거나 로그인·권한 오류인 경우에는 중복 실행을 막기 위해 재시도하지 않습니다.
...
워크플로우가 여러 파일·Notion 도구를 순차 호출할 때 `중계 HTTP 410`이 나오면 중계 작업이 만료된 것입니다. 최신 릴레이의 기본 작업 제한은 30분이고 워커가 15초마다 lease를 연장하므로 작업 중에는 제한이 계속 밀립니다. NAS가 이전 릴레이라면 `JOB_TIMEOUT_MS=1800000` 환경 변수를 설정하고 `/mcp`·`/worker/*` 프록시 타임아웃도 30분 이상으로 맞추세요.

#### Unity CLI 경로 오류

`Unable to write to User Settings because schoolCode.unityExecutable is not a registered configuration` 오류가 나오면 이전 VSIX가 설치된 상태입니다. `school-code-0.20.0.vsix`로 업데이트하고 **Developer: Reload Window**를 실행하세요. 0.8.1부터 Unity 경로는 VS Code User Settings를 갱신하지 않고 확장 전용 상태에 저장하므로 해당 설정 오류가 발생하지 않습니다. `Unity.exe`가 PATH에 있으면 그대로 입력하고, 아니면 `C:\Program Files\Unity\Hub\Editor\버전\Editor\Unity.exe`처럼 전체 경로를 입력합니다.
...
브리지 도구는 씬·GameObject·컴포넌트 조회와 수정, Scene/Game 캡처, 모델 미리보기, Play Mode, Console 로그, 프로젝트 상태, Prefab·Material·Animator 작업을 제공합니다. loopback과 Bearer 토큰을 사용하고 C#·셸 명령을 실행하지 않습니다. 캡처·조회는 읽기 작업이고 씬/오브젝트/재생 상태를 바꾸는 도구는 쓰기 승인 대상입니다. 자세한 설치는 `unity-editor/README.md`를 참고하세요.

주요 Unity Editor 도구는 다음과 같습니다.

- `unity_capture_scene`, `unity_capture_game`: 현재 Scene/Game 화면을 PNG MCP 이미지로 반환
- `unity_model_preview`: `Assets/` 아래 모델의 Unity 미리보기 반환
- `unity_play`, `unity_pause`, `unity_stop`: Play Mode 제어
- `unity_get_console_logs`, `unity_project_status`: 최근 로그, 컴파일·임포트 상태, 현재 씬, 빌드 대상·씬 확인
- `unity_add_component`, `unity_remove_component`, `unity_duplicate_gameobject`, `unity_delete_gameobject`, `unity_move_gameobject`: Undo 가능한 기본 편집
- `unity_instantiate_prefab`, `unity_assign_material`, `unity_get_animator_info`, `unity_set_animator_parameter`: 자주 쓰는 에셋·Animator 작업

FBX·OBJ·GLB·GLTF는 Workspace의 `list_model_assets`와 `read_model_metadata`로 파일을 전송하지 않고 기본 메타데이터를 확인합니다. 실제 모습을 볼 때는 Unity가 임포트한 뒤 `unity_model_preview`를 사용합니다.
