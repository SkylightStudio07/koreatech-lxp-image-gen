...
async function browserWorker(port,key) {
  ...
  while(active){try{
    ...
    try{
      ...
      const r=await fetch('/api/AiCA/api/v1'+job.route,{method:job.method,credentials:'include',redirect:'error',headers:h,body:requestBody,signal:AbortSignal.any([current.signal,AbortSignal.timeout(35*60*1000)])});
