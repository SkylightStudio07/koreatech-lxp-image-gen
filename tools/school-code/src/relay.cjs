...
const toolDefinitions={
  ...
  read_skill:{description:'Read one project skill from .school-code/skills/<name>/SKILL.md after user approval. Other skills directories are not searched.',schema:{name:z.string().min(1).max(64)}},
...
function createRelay({mcpToken,workerToken,host='127.0.0.1',port=18880,timeoutMs=120000}={}){
  ...
  function mcp(){const server=new McpServer({name:'koreatech-workspace',version:'0.3.0'});for(const [name,d]of Object.entries(toolDefinitions))server.registerTool(name,{description:d.description,inputSchema:d.schema,annotations:{readOnlyHint:name!=='propose_edit',destructiveHint:name==='propose_edit',openWorldHint:name.startsWith('notion_')}},async (args,extra)=>{try{return {content:[{type:'text',text:JSON.stringify(await enqueue(name,args,extra.signal))}]};}catch(e){return {isError:true,content:[{type:'text',text:e.message}]};}});return server;}
