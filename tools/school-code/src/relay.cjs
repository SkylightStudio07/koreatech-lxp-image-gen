...
function createRelay({mcpToken,workerToken,host='127.0.0.1',port=18880,timeoutMs=120000,authFile='',registrationCode='',allowRegistration=true,publicUrl='',pairingTtlMs=10*60*1000}={}){
  ...
  async function authRequest(req,res,url){
    ...
    if(url.pathname==='/auth/pair'&&req.method==='GET'){const pair=auth.pairByCode(url.searchParams.get('code'));if(!pair)return json(res,410,{error:'연결 코드가 만료되었거나 올바르지 않습니다.'});res.setHeader('Content-Type','text/html; charset=utf-8');return res.end(pairingPage({code:pair.code,user:auth.userFromRequest(req),expiresAt:pair.expiresAt}));}
