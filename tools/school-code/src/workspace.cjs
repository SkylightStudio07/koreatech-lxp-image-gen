...
const MAX_MODEL_METADATA_BYTES=64*1024*1024;
const MODEL_EXCLUDED_DIRECTORIES=VISUAL_EXCLUDED_DIRECTORIES;
...
function modelExtension(input){const ext=path.extname(input).toLowerCase();return MODEL_EXTENSIONS.has(ext)?ext:null;}
function modelCountsFromJson(json){const count=name=>Array.isArray(json?.[name])?json[name].length:0;return {meshes:count('meshes'),nodes:count('nodes'),materials:count('materials'),animations:count('animations'),skins:count('skins'),scenes:count('scenes')};}
function parseGltf(buffer,ext){let json;if(ext==='.glb'){if(buffer.length<20||buffer.subarray(0,4).toString('ascii')!=='glTF')throw Error('GLB 헤더를 읽을 수 없습니다.');const version=buffer.readUInt32LE(4),length=buffer.readUInt32LE(8);if(version!==2||length>buffer.length)throw Error('지원하는 GLB 버전이 아닙니다.');const chunkLength=buffer.readUInt32LE(12),chunkType=buffer.readUInt32LE(16);if(chunkType!==0x4e4f534a||chunkLength>buffer.length-20)throw Error('GLB JSON 청크를 읽을 수 없습니다.');json=JSON.parse(buffer.subarray(20,20+chunkLength).toString('utf8').replace(/\0+$/,''));return {format:'glb',version,bytes:length,...modelCountsFromJson(json),generator:json.asset?.generator||null};}json=JSON.parse(buffer.toString('utf8'));return {format:'gltf',version:json.asset?.version||null,...modelCountsFromJson(json),generator:json.asset?.generator||null};}
function parseObj(text){const count=prefix=>{const match=text.match(new RegExp(`(?:^|\\n)\\s*${prefix}(?:\\s|$)`,'gm'));return match?match.length:0;};const objects=(text.match(/(?:^|\n)\s*(?:o|g)\s+[^\n]*/g)||[]).length;return {format:'obj',vertices:count('v'),normals:count('vn'),uvs:count('vt'),faces:count('f'),objects,materials:(text.match(/(?:^|\n)\s*mtllib\s+/g)||[]).length};}
function parseFbx(buffer){const binary=buffer.subarray(0,21).toString('ascii')==='Kaydara FBX Binary';if(binary){return {format:'fbx',binary:true,version:buffer.length>=27?buffer.readUInt32LE(23):null};}const text=buffer.toString('utf8');const count=(label)=>{const m=text.match(new RegExp(`${label}:\\s*[^\\n]+`,'i'));return m?1:0;};return {format:'fbx',binary:false,objects:(text.match(/\bModel::/g)||[]).length,meshes:(text.match(/\bGeometry::/g)||[]).length,animations:(text.match(/\bAnimationStack::/g)||[]).length,has_vertices:Boolean(count('Vertices')),has_polygon_indices:Boolean(count('PolygonVertexIndex'))};}
async function readModelMetadata(root,input){const ext=modelExtension(input);if(!ext)throw Error('지원하는 3D 형식은 FBX, OBJ, GLB, GLTF입니다.');const file=await safePath(root,input),stat=await fs.stat(file);if(stat.size<=0)throw Error('빈 3D 에셋은 읽을 수 없습니다.');if(stat.size>MAX_MODEL_METADATA_BYTES)throw Error('3D 에셋 메타데이터는 64 MiB 이하 파일만 읽을 수 있습니다.');const sample=Buffer.alloc(Math.min(stat.size,1024));const handle=await fs.open(file,'r');let bytesRead=0;try{({bytesRead}=await handle.read(sample,0,sample.length,0));}finally{await handle.close();}let parsed;if(ext==='.fbx')parsed=parseFbx(sample);else if(stat.size>16*1024*1024)throw Error('OBJ/GLTF/GLB 메타데이터는 16 MiB 이하 파일만 읽을 수 있습니다.');else {const buffer=await fs.readFile(file);parsed=ext==='.obj'?parseObj(buffer.toString('utf8')):parseGltf(buffer,ext);}return {path:input.replace(/\\/g,'/'),size:stat.size,extension:ext,mime:MIME[ext]||'application/octet-stream',sha256:stat.size<=256*1024*1024?await fileHash(file):null,modified_at:stat.mtime.toISOString(),...parsed,unity_meta:Boolean(await unityTextureMeta(root,input))};}
async function listModelAssets(root,input='',limit=200){if(!Number.isInteger(limit)||limit<1||limit>500)throw Error('3D 에셋 목록은 1~500개까지 요청할 수 있습니다.');const start=await safePath(root,input,{directory:true});const out=[];let visited=0;async function walk(dir){let entries;try{entries=await fs.readdir(dir,{withFileTypes:true});}catch(e){if(['EACCES','EPERM','ENOENT'].includes(e.code))return;throw e;}for(const e of entries){if(out.length>=limit||++visited>10000)return;if(DENIED.test(e.name)||e.isSymbolicLink()||(e.isDirectory()&&(MODEL_EXCLUDED_DIRECTORIES.has(e.name)||e.name.startsWith('.'))))continue;const p=path.join(dir,e.name),rel=path.relative(root,p).split(path.sep).join('/');if(e.isDirectory())await walk(p);else if(e.isFile()&&modelExtension(e.name)){try{out.push(await readModelMetadata(root,rel));}catch{/* Ignore malformed or oversized model assets in an inventory. */}}}}await walk(start);return {assets:out.sort((a,b)=>a.path.localeCompare(b.path)),truncated:out.length>=limit||visited>10000};}
