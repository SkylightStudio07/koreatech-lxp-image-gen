...
async function injectWorker(tabId, url) {
  ...
  if(tabs[0]?.id!==tabId)return false;
  const port = await getPort();
  try {
    await chrome.scripting.executeScript({
      target: { tabId },
      world: 'MAIN',
      func: configuredPort => { window.__schoolCodeConnectorPort = configuredPort; },
      args: [port]
    });
...
async function workerState(tabId) {
  try {
    const [{ result }] = await chrome.scripting.executeScript({
      ...
      world: 'MAIN',
      func: () => {
        const state = window.__schoolCodeWorker;
...
async function bridgeStatus(port) {
  try {
    ...
    const data=await response.json(); if(data.product!=='school-code')throw new Error('WRONG_BRIDGE'); return data;
  } catch {
...
async function updateBadge() {
  ...
  if (!status.bridge.broker) {
    ...
    text = 'ON'; color = '#16a34a'; title = '학교 VS Code 연결됨';
  } else if (status.tab) {
...
async function reconnectAll() {
  const tabs = await schoolTabs();
  await Promise.all(tabs.slice(0,1).map(tab => tab.id === undefined ? false : injectWorker(tab.id, tab.url)));
...
chrome.alarms.onAlarm.addListener(alarm => { if (alarm.name === 'refresh-status') reconnectAll(); });
