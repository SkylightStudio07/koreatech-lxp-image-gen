...
구성원에게는 GitHub 저장소의 소스 링크보다 **최신 GitHub Release 페이지 링크**를 전달하세요. Release에서 `school-code-0.20.0.vsix`와 `school-code-connector-0.3.0.zip`을 내려받아 설치하면 됩니다. 저장소 소스는 개발·검토용이고, Release 첨부 파일이 일반 구성원의 설치용입니다.
...
- 최신 GitHub Release의 `school-code-0.20.0.vsix`
...
code --install-extension .\tools\school-code\school-code-0.20.0.vsix --force
...
```text
skylight-local.koreatech-school-code@0.20.0
```
...
4. Release에서 내려받은 `school-code-0.20.0.vsix`를 선택합니다.
...
승인 페이지가 계속 버튼만 보여 주거나 MCP 카드가 즉시 `오류`로 바뀌면 VS Code를 `Developer: Reload Window`로 다시 로드하고 최신 `0.20.0` VSIX를 설치하세요. NAS 릴레이도 최신 `dist/relay.cjs`로 재빌드해야 승인 토큰을 잃어버린 경우 재시도할 수 있습니다.
...
MCP 리소스 화면에 `연결 확인됨 · 도구 66개`가 보여도, 에이전트의 워크플로우 MCP 노드가 서버를 선택하지 않았다면 모델에는 도구가 전달되지 않습니다.

1. 학교 AI의 MCP 상세 화면에서 `BCSD School Code Workspace`를 사용할 에이전트를 선택합니다.
2. 에이전트 편집 → **워크플로우**로 이동합니다.
3. **MCP 도구** 노드를 선택하고 `MCP 서버`가 `School Code Workspace`로 지정되어 있는지 확인합니다. `선택...`으로 남아 있으면 도구를 호출할 수 없습니다.
4. 노드 속성에 `도구 66개`와 `workspace_info`, `read_image`, `generate_image_asset`, `save_image_asset` 등이 보이는지 확인합니다.

이미지 생성은 별도 툴콜 문법을 입력하지 않습니다. `School Image Producer` 에이전트를 선택하고 다음처럼 요청합니다.
...
`generate_image_asset`이 목록에 없으면 NAS 릴레이를 최신 릴레이 소스로 재빌드하고 학교 리소스의 MCP를 해제 후 다시 등록한 다음, 워크플로 MCP 노드에서 `School Code Workspace`를 다시 선택합니다. 도구가 목록에 있는데도 이미지 워크플로가 60초에서 끊기면 VS Code 0.20.0 이상에서 `School Image Producer`를 선택하세요. 이 확장은 해당 에이전트만 학교 Workflow API를 우회해 로그인된 Chrome 세션의 이미지 호출을 직접 실행합니다.
...
페어링 토큰을 재발급하면 VS Code에서 **MCP 인증 JSON 복사**를 다시 누르고, 학교 리소스의 기존 항목을 **편집**해서 인증값만 교체합니다. 기존 리소스를 삭제하고 같은 이름으로 다시 만들면 학교가 새 리소스 ID를 발급하므로, 에이전트 워크플로우의 MCP 노드가 `선택...`으로 풀릴 수 있습니다. 이미 삭제했다면 위 2~5단계처럼 각 에이전트의 MCP 노드에서 새 리소스를 다시 선택하고 저장합니다.
...
- [ ] VS Code에 `skylight-local.koreatech-school-code@0.20.0`이 설치됨
