...
MCP 리소스 화면에 `연결 확인됨 · 도구 61개`가 보여도, 에이전트의 워크플로우 MCP 노드가 서버를 선택하지 않았다면 모델에는 도구가 전달되지 않습니다.
...
4. 노드 속성에 `도구 61개`와 `workspace_info`, `read_image`, `save_image_asset` 등이 보이는지 확인합니다.
